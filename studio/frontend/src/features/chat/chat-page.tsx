...
export function ChatPage({
  ...
  useEffect(() => {
    ...
    void authFetch("/api/models/cached-models")
      ...
      .then((data: { cached: { repo_id: string; size_bytes?: number }[] }) => {
        ...
        // repo_id -> size, so a default voice shows a size tag even though it
        // isn't built from this scan directly.
  ...
  const ttsModels = useMemo<LoraModelOption[]>(() => {
    ...
    const all = [...fromLoras, ...cachedGgufs, ...voiceDefaults];
    // Attach on-disk size (from the cached-models scan) where the entry doesn't
    // already carry one, so a default voice still shows a size tag.
    return all.map((m) =>
      m.sizeBytes != null ? m : { ...m, sizeBytes: cachedSizes[m.id.toLowerCase()] },
    );
  }, [loraModels, cachedGgufs, cachedSizes]);
