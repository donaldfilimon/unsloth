...
import { registerVoiceResume } from "@/features/chat/voice/voice-loop-bridge";
import {
  ORB_IDLE_GRADIENT,
  VoiceOrb,
  orbConfig,
...
const VoiceEngine: FC = () => {
  ...
  useEffect(() => {
    ...
    if (voiceSlotLoading) { clearSynthGap(); setVoiceOrbState("loading"); return; }
...
const ComposerToolsMenu: FC<{
  ...
  // Voice mode needs BOTH halves, and the listening half is the narrower one.
  // Speaking: the browser's speechSynthesis, or a loaded TTS codec. Listening:
  // the streaming Web Speech engine specifically, because the loop's turn-taking
  // watches the transcript grow. A batch engine (local or custom transcription)
  // holds its transcript until end-of-utterance, so the orb would sit on
  // "listening" and nothing would ever send. Gating on TTS alone offered Voice in
  // browsers with speechSynthesis but no SpeechRecognition, where it could be
